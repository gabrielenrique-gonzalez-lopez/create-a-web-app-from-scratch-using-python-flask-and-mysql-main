...
def test_create(client, auth):
    ...
    assert response.headers["Location"] == "http://localhost/blog/create"
...
def test_update(client, auth):
    ...
    response = client.post("/1/update", data={"title": "updated", "content": ""})
    assert response.headers["Location"] == "http://localhost/blog/1/update"

    response = client.post(
        "/1/update", data={"title": "updated", "content": "test content"}
    )
    assert response.headers["Location"] == "http://localhost/blog/index"
    with client:
        response = client.get("/")
